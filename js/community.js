// ============================================
// HARAZIMIYYA FORUM - COMMUNITY CHAT
// Public community chat for all members
// ============================================

console.log("💬 Community Chat loading...");
...
document.addEventListener('DOMContentLoaded', function() {
    console.log("DOM loaded, initializing community chat...");
    initializeChat();
});
...
async function loadCommunityMessages() {
    try {
        ...
        if (!messages || messages.length === 0) {
            messagesEl.innerHTML = '<div class="empty-chat"><i class="fas fa-comments"></i><h3>No messages yet</h3><p>Be the first to send a message!</p></div>';
            return;
        }
        ...
        if (isSent) {
            ...
            html += `
                <div class="message received" data-message-id="${msg.id}" data-sender-id="${msg.sender_id}">
                    <small>${escapeHtml(senderName)}${crown}</small>
                    ${renderQuotedMessage(msg.parent)}
                    <div class="message-content">${renderMessageContent(msg)}</div>
                    <span class="time">${timeStr}</span>
                </div>
...
function setupRealtimeSubscription() {
    ...
    messagesSubscription = window.supabase
        ...
        .on('postgres_changes', { 
            ...
            schema: 'public', 
            table: 'chat_messages',
            filter: 'receiver_id=is.null AND group_id=is.null'
        }, payload => {
...
async function sendMessage() {
    ...
    try {
        const messageData = {
            ...
            read_at: null,
            receiver_id: null,
            group_id: null  // Community chat has both null
        };
